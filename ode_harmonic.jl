...
# ╔═╡ 38907164-4ae8-11ed-01b8-87dc467a2308
using DifferentialEquations, Plots

# ╔═╡ e77fc156-aa97-42df-bae2-5470d8ae5449
md"""
# Simple Harmonic Motion

**What if** you drill a hole from the North Pole to the South Pole,
and jump into it?

According to the gravity formula:

```math
F = \frac{G M m}{r^2}
```

where $M = \frac{4}{3} \rho \pi r^3$

```math
\therefore
F = \frac{4 G m \rho \pi r^3}{3 r^2} = \frac{4}{3} G  m \rho \pi r
```

therefore the equation is:

```math
\frac{d^2}{d t^2} r = -k r
```

where
$k = \frac{4}{3} G \rho \pi
\approx 1.537 \times 10^{-6}$

Assume a solution will be proportional to $e^{\lambda t}$.

Substitute $r(t)$ with $e^{\lambda t}$:

```math
\begin{cases}
	\frac{d^2}{d t^2} r = \lambda^{2} e^{\lambda t}
	\\
	-kr = -k e^{\lambda t}
\end{cases}
```
...
\therefore
\begin{cases}
	\lambda_1 = i \cdot 1.23976 \cdot 10^{-3}
	\\
	\lambda_2 = - i \cdot 1.23976 \cdot 10^{-3}
\end{cases}
```
...
```math
\frac{d^2} {dt^2} r
= C_1 \lambda_1^{2} e^{\lambda_1 t} C_2 \lambda_2^{2} e^{\lambda_2 t}
= -k (C_1 e^{\lambda_1 t} + C_2 e^{\lambda_2 t})
= -k r
```

let $\mu = 1.23976 \cdot 10^{-3}$, thus:

```math
r(t) = C_1 e^{i \mu t} + C_2 e^{-i \mu t}
```

```math
r(t)
= C_1 (cos(\mu t) + i \cdot sin(\mu t)) + C_2 (cos(\mu t) - i \cdot sin(\mu t))
```

```math
r(t)
= (C_1 + C_2) cos(\mu t) + (C_1 - C_2) i \cdot sin(\mu t))
```
...
```math
\therefore
r(t) = 6.371 \cdot 10^{6} \cdot cos(1.23976 \cdot 10^{-3} \cdot t)
```
"""

# ╔═╡ c0db4d3a-f527-4262-b0e5-789d9266a62f
begin
	ρ = 5.5e3
	G = 6.67e-11
	dr₀ = 0.0
...
# ╔═╡ 64ff6e26-364b-4513-a6e8-c1bd820705df
function ode(dr, r, p, t)
	-K * r
end

# ╔═╡ 753870eb-2b16-4eae-8cb8-49c6ddc8c8b5
prob = SecondOrderODEProblem(ode, dr₀, r₀, (t₁, t₂))

# ╔═╡ bbc43c22-7d13-4db6-bb0b-32560a4b681a
sol = solve(prob) # TsitPap8())# Tsit5())#, DPRKN12())# Vern9())

# ╔═╡ 535c060b-520d-4ceb-995a-c0a6823c013b
begin
	r̂(t) = sol(t)[2]
	dr̂(t) = sol(t)[1]
end

# ╔═╡ 017164e9-3c9e-4d7c-8b80-76ccf66f2e9f
r(t) = r₀ * cos(1.23976e-3 * t)

# ╔═╡ 12210e14-051b-44fb-a65e-1db69d6067b5
plot(t₁:t₂,
	[r̂, r],
	title = "Fitting with r(t)",
	label = ["r̂" "r"],
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "f268647427ef7609d00e7eccdb7229ed18c7d1ff"
